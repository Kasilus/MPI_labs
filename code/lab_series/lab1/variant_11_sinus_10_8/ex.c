...
/* n = term_number, x = value */
double calc_series_term(int term_number, double value)
{
  // return pow(-1, term_number) * pow(value, 2 * term_number + 1) / factorial(2 * term_number + 1);
  return pow(-1, term_number - 1) * pow(value, 2 * term_number - 1) / factorial(2 * term_number - 1);
}
